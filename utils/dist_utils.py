...
def collate_weights(delta_curr):
    ...
    for l in range(len(delta_curr)):
        ...
        if l == 0:
            ...
            delta_curr_w = np.concatenate((delta_curr_w, flat_layer))
        elif (l + 1) % 2 == 0:
            delta_curr_b = np.concatenate((delta_curr_b, flat_layer))
...
def plr_constrain(loss1, mal_loss1, agent_model, plr_loss, t):
    """
    add regulation to the loss optimization. loss = loss + rho*|w-w_global|
    :param loss1:
    :param mal_loss1:
    ...
    loss = loss1
    mal_loss = mal_loss1 + rho * plr_loss

    return loss, loss2, mal_loss


def weight_and_plr_constrain(loss1, mal_loss1, agent_model, constrain_weights, plr_loss,  t):
    """
    add regulation to the loss optimization. loss = loss + rho*|w-w_global|
    :param loss1:
    :param mal_loss1:
    ...
    for layer in agent_model.layers:
        ...
        for weight in layer.weights:
            ...
            layer_count += 1
            counter += 1
    loss = loss1 + rho * loss2 + 0.1 * plr_loss
    mal_loss = mal_loss1
