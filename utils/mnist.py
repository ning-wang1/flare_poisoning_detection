...
def data_mnist(one_hot=True):
    ...
    if gv.args.dataset == 'MNIST':
        ...
        X_train, y_train = load_fmnist('data/fashion-mnist', kind='train')
        X_test, y_test = load_fmnist('data/fashion-mnist', kind='t10k')
...
def modelB():
    ...
    model.add(Dropout(0.2, input_shape=(gv.IMAGE_ROWS,
                                        gv.IMAGE_COLS,
                                        gv.NUM_CHANNELS)))
    model.add(Conv2D(64, 8, 8, subsample=(2, 2), border_mode='same'))
    model.add(Activation('relu'))

    model.add(Conv2D(128, 6, 6, subsample=(2, 2), border_mode='valid'))
    model.add(Activation('relu'))
