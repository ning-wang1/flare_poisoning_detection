...
from utils.dist_utils import est_accuracy, weight_constrain, plr_constrain, weight_and_plr_constrain
...
def benign_train(x, y, agent_model, logits, X_shard, Y_shard, shared_weights):
    """
    ...
    if args.optimizer == 'adam':
        ...
        print('unknown optimizer, use SGD instead')
        optimizer = tf.train.GradientDescentOptimizer(learning_rate=args.eta).minimize(loss)
    ...
    for step in range(int(num_mal_epochs * shard_size / args.B)):
        ...
        _, loss_val = temp_sess.run([optimizer, loss], feed_dict={x: X_batch, y: Y_batch_uncat})
    ...
    if 'MNIST' in args.dataset:
        ...
        layer_idx = 11  # 13
    layer_model = Model(agent_model.inputs, agent_model.layers[layer_idx].output)
    ...
    for step_temp in range(num_steps_temp):
        ...
        X_batch = X_shard[offset_temp: (offset_temp + args.B)]
        Y_batch = Y_shard[offset_temp: (offset_temp + args.B)]
        Y_batch_uncat = np.argmax(Y_batch, axis=1)
        loss_val_shard += temp_sess.run(loss, feed_dict={x: X_batch, y: Y_batch_uncat})

        plr = temp_sess.run(layer_output, feed_dict={x: X_batch, y: Y_batch_uncat})
        plr_groupby_cat, count = sum_groupby_cat(plr, Y_batch_uncat, cat)
        if step_temp == 0:
            plr_arr = plr_groupby_cat
        else:
            plr_arr += plr_groupby_cat
        counts += count

    for i in range(len(cat)):
        plr_arr[i] = plr_arr[i] / counts[i]

    plr_out = np.zeros([Y_shard.shape[0], plr_arr.shape[1]])
    for step_temp in range(num_steps_temp):
        offset_temp = (offset + step_temp * args.B) % (shard_size - args.B)
        Y_batch = Y_shard[offset_temp: (offset_temp + args.B)]
        Y_batch_uncat = np.argmax(Y_batch, axis=1)
        plr_out[step_temp*args.B: (step_temp+1)*args.B, :] = plr_arr[list(Y_batch_uncat), :]
    ...
    return final_delta, loss_val_shard, plr_arr


def sum_groupby_cat(x, y, cat):

    x_sum_groupby_c = np.zeros([len(cat), x.shape[1]])
    counts = np.zeros(len(cat))
    for c in cat:
        idx_c = list(np.where(y == c)[0])
        y_c = x[idx_c, :]
        x_sum_groupby_c[c, :] = np.sum(y_c, axis=0)
        counts[c] = len(idx_c)

    return x_sum_groupby_c, counts


def data_poison_train(sess, optimizer, loss, x, y, X_shard, Y_shard, mal_data_X, mal_data_Y, num_steps, start_offset):
    """
    ...
    index_rand = np.random.permutation(shard_size)
    ...
    while step < num_steps:
        ...
        step += 1
        if step % 100 == 0:
            loss_val = sess.run([loss], feed_dict={x: X_batch, y: Y_batch_uncat})
            mal_loss_val = sess.run([loss], feed_dict={x: mal_data_X, y: mal_data_Y})
            print('Benign: Loss - %s; Mal: Loss - %s' % (loss_val, mal_loss_val))


def concat_train(sess, optimizer, loss, mal_optimizer, mal_loss, x, y, X_shard, Y_shard, mal_data_X, mal_data_Y,
    ...
    while step < num_steps:
        ...
        if step % 100 == 0:
            loss_val = sess.run([loss], feed_dict={x: X_batch, y: Y_batch_uncat})
            mal_loss_val = sess.run([mal_loss], feed_dict={x: mal_data_X, y: mal_data_Y})
            print('Benign: Loss - %s; Mal: Loss - %s' % (loss_val, mal_loss_val))
        step += 1


def alternate_train(sess, t, optimizer, loss, mal_optimizer, mal_loss, x, y, X_shard, Y_shard,
                    mal_data_X, mal_data_Y, agent_model, num_steps, start_offset, loss1=None, loss2=None):
    """
    ...
    while step < num_steps:
        ...
        if step < num_steps:
            for l_step in range(num_local_steps):
                ...
                if 'dist' in args.mal_strat:
                    loss1_val, loss2_val, loss_val = sess.run([loss1, loss2, loss],
                                                              feed_dict={x: X_batch, y: Y_batch_uncat})
                    sess.run([optimizer], feed_dict={x: X_batch, y: Y_batch_uncat})
                else:
                    loss_val = sess.run([loss], feed_dict={x: X_batch, y: Y_batch_uncat})
                    sess.run([optimizer], feed_dict={x: X_batch, y: Y_batch_uncat})

        mal_loss_val_bef = sess.run([mal_loss], feed_dict={x: mal_data_X, y: mal_data_Y})

        # Malicious step, only if mal loss is non-zero
        if step >= 0 and any(mal_loss_val_bef) > 0.0:
        ...
        mal_loss_val_aft = sess.run([mal_loss], feed_dict={x: mal_data_X, y: mal_data_Y})

        if step % 100 == 0 and 'dist' in args.mal_strat:
        ...
        elif step % 100 == 0 and 'dist' not in args.mal_strat:
            print('Benign: Loss - %s; Mal: Loss_bef - %s, Loss_aft - %s' %
                  (loss_val, mal_loss_val_bef, mal_loss_val_aft))
        # if step % 100 == 0 and t < 5:
        #     np.save(gv.dir_name + 'mal_delta_t%s_step%s.npy' % (t, step), delta_mal_local)
...
def alternate_train_mal_first(sess, t, optimizer, loss, mal_optimizer, mal_loss, x, y, X_shard, Y_shard,
                    mal_data_X, mal_data_Y, agent_model, num_steps, start_offset, loss1=None, loss2=None):
    """
    train adversary model using alternate training method: perform num_steps of benign train, and then one step of
    malicious train
...
def mal_single_algs(x, y, logits, agent_model, shared_weights, sess, mal_data_X, mal_data_Y, t, mal_visible,
                    X_shard, Y_shard, plr):
    """
    ...
    # Initial start weights at agent: 1. the global model; or 2. the global weight plus other agent update
    ...
    # The constrain weights
    constrain_weights = shared_weights
    if 'dist' in args.mal_strat:
        if 'dist_oth' in args.mal_strat and t >= 1:
            constrain_weights = shared_weights + delta_other_prev
        else:
            final_delta, _, ben_plr = benign_train(x, y, agent_model, logits, X_shard, Y_shard, shared_weights)
            constrain_weights = shared_weights + final_delta
            K.set_session(sess)
    elif 'add_ben' in args.mal_strat:
        ben_delta, loss_val_shard, ben_plr = benign_train(x, y, agent_model, logits, X_shard, Y_shard, shared_weights)
    elif 'unlimited' in args.mal_strat:
        ben_delta, loss_val_shard, ben_plr = benign_train(x, y, agent_model, logits, X_shard, Y_shard, shared_weights)

    # Loss Function. total loss function including: loss1, mal_loss and loss2
    loss1 = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=logits))
    mal_loss1 = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=logits))
    ...
    if 'dist' in args.mal_strat:
        ...
        constrain_plr = tf.convert_to_tensor(ben_plr, dtype=tf.float32)
        plr_loss = tf.nn.l2_loss(tf.reduce_mean(plr - tf.gather(constrain_plr, y), 0))
        if 'adaptive' in args.mal_strat:
            loss, loss2, mal_loss = weight_and_plr_constrain(
                loss1, mal_loss1, agent_model, constrain_weights, plr_loss, t)
        else:
    ...
    # initialize optimizer for malicious goal and benign goal
    if 'adam' in args.optimizer:
        optimizer = tf.train.AdamOptimizer(learning_rate=args.eta).minimize(loss)
        mal_optimizer = tf.train.AdamOptimizer(learning_rate=args.eta).minimize(mal_loss)
    elif 'sgd' in args.optimizer:
        optimizer = tf.train.GradientDescentOptimizer(learning_rate=args.eta).minimize(loss)
        mal_optimizer = tf.train.GradientDescentOptimizer(learning_rate=args.eta).minimize(mal_loss)
    ...
    # initial mal data and mal_delta (update)
    ...
    # 1. Compute mal_delta. the optimization only consider mal_loss without considering loss.
    ...
    # 2. Compute mal_delta. The optimization consider both loss (for benign purpose) and mal_loss.
        ...
        if 'alternate' in args.mal_strat:
            if 'unlimited' not in args.mal_strat:
                delta_mal_ret = alternate_train(sess, t, optimizer, loss, mal_optimizer, mal_loss, x, y,
                                                X_shard, Y_shard, mal_data_X, mal_data_Y, agent_model, num_steps,
                                                start_offset, loss1, loss2)
                # delta_mal_ret = alternate_train_mal_first(sess, t, optimizer, loss, mal_optimizer, mal_loss, x, y,
                #                                 X_shard, Y_shard, mal_data_X, mal_data_Y, agent_model, num_steps,
                #                                 start_offset, loss1, loss2)
            elif 'unlimited' in args.mal_strat:
                # train until loss matches that of benign trained
                alternate_train_unlimited(sess, t, optimizer, loss, mal_optimizer, mal_loss, x, y, logits,
                                          X_shard, Y_shard, mal_data_X, mal_data_Y, agent_model, num_steps,
                                          start_offset, loss_val_shard, loss1, loss2)
        elif 'concat' in args.mal_strat:
            # training with concatenation
            concat_train(sess, optimizer, loss, mal_optimizer, mal_loss, x, y, X_shard, Y_shard, mal_data_X,
            ...
            data_poison_train(sess, optimizer, loss,  x, y, X_shard, Y_shard, mal_data_X, mal_data_Y,
                              num_steps, start_offset)
    # Boost the mal_delta or not
...
def mal_all_algs(x, y, logits, agent_model, shared_weights, sess, mal_data_X, mal_data_Y):
    """
    ...
    loss = -1.0 * tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=logits))
    optimizer = tf.train.AdamOptimizer(learning_rate=args.eta).minimize(loss)
    # optimizer = tf.train.GradientDescentOptimizer(learning_rate=args.eta).minimize(loss)
    ...
    for step in range(num_mal_epochs * data_len / gv.BATCH_SIZE):
        ...
        sess.run(optimizer, feed_dict={x: X_batch, y: Y_batch_uncat})
        if step % 10 == 0:
            curr_loss = sess.run(loss, feed_dict={x: X_batch, y: Y_batch_uncat})
...
def mal_agent(mal_index, X_shard, Y_shard, mal_data_X, mal_data_Y, t, gpu_id, return_dict, mal_visible, X_test, Y_test):
    """
    ...
    # obtain malicious update
    if t >= args.mal_delay and holdoff_flag == 0:
        if args.mal_obj == 'all':
            final_delta = mal_all_algs(x, y, logits, agent_model, shared_weights, sess, mal_data_X, mal_data_Y)
        elif args.mal_obj == 'single' or 'multiple' in args.mal_obj or 'target_backdoor' in args.mal_obj:
            final_delta, penul_delta = mal_single_algs(x, y, logits, agent_model, shared_weights, sess, mal_data_X,
                                                       mal_data_Y, t, mal_visible, X_shard, Y_shard, plr)
    elif t < args.mal_delay or holdoff_flag == 1:
        print('Delay/Hold-off')
        final_delta, _, _ = benign_train(x, y, agent_model, logits, X_shard, Y_shard, shared_weights)

    final_weights = shared_weights + final_delta
    agent_model.set_weights(final_weights)

    # evaluate malicious agent
    print('-------Eval at mal agent-------{}'.format(mal_index))
    if 'single' in args.mal_obj:
        target, target_conf, actual, actual_conf = mal_eval_single(mal_data_X, mal_data_Y, final_weights)
        # print(final_weights[9][0:5])
    ...
    return_dict[str(mal_index)] = np.array(final_delta)
    ...
    if 'auto' in args.mal_strat or 'multiple' in args.mal_obj:
        ...
        if 'single' in args.mal_obj:
            ...
            print('Penul weights --,For iter %s, Target:%s with conf. %s, Curr_pred on malicious model:%s with conf. %s'
    ...
    agents.append(mal_index)
    return_dict['agents'] = agents

    return


def mal_agent_copy(mal_index, X_shard, Y_shard, mal_data_X, mal_data_Y, t, gpu_id, return_dict, mal_visible, X_test, Y_test):
    args = gv.args
    shared_weights = np.load(gv.dir_name + 'global_weights_t%s.npy' % t, allow_pickle=True)
    ...
    # obtain malicious update
    final_delta = np.load(gv.dir_name + 'mal_delta_t%s.npy' % t, allow_pickle=True)
    final_weights = shared_weights + final_delta
    agent_model.set_weights(final_weights)
